...
constexpr uint32_t WEB_LEASE_MS = 2500;
constexpr int32_t TRANSFER_Z_STEP_MM = 2;
constexpr int32_t MAX_TRANSFER_Z_MM = 30;
constexpr int32_t X_HOME_MM = 500;
constexpr int32_t Z_HOME_MM = 400;
...
enum OperationType : uint8_t { OP_NONE, OP_PICK, OP_PLACE, OP_HOME };
enum SystemState : uint8_t {
  BOOT, HOME_Y_SEARCH, HOME_Y_RETRACT, HOME_TOWER_START, HOME_TOWER_WAIT,
  READY, VALIDATE, MOVE_TOWER_START, MOVE_TOWER_WAIT, EXTEND_Y,
  TRANSFER_CHECK, TRANSFER_MOVE_START, TRANSFER_MOVE_WAIT, RETRACT_Y,
  VERIFY_RACK, SUCCESS, STOPPING, ERROR_STATE
};
...
<button onclick="cmd('/api/home')">Home system</button><button class="danger" onclick="cmd('/api/stop')">STOP</button></div>
...
void updateOperation() {
  ...
  if(state==VALIDATE) {
    ...
    if(operation==OP_PICK && !rackOccupied[selectedSlot]) { fail("Cannot pick from an empty slot"); return; }
    if(operation==OP_PLACE && rackOccupied[selectedSlot]) { fail("Cannot place into an occupied slot"); return; }
    ...
    if(fabs(currentY)>Y_RETRACTED_TOLERANCE_MM) { fail("Fork must be retracted before tower travel"); return; }
  ...
  if(state==STOPPING) { yMoving=false; digitalWrite(STEP_PIN,LOW); fail("Stopped by operator or lost webpage"); }
}
...
void setupWeb() {
  ...
  auto operationHandler=[](OperationType requested){
    ...
    selectedSlot=slot;operation=requested;errorMessage="";statusMessage=requested==OP_PICK?"Pick started":"Place started";lastBrowserHeartbeat=millis();state=VALIDATE;stateStartedMs=millis();server.send(202,"text/plain","Accepted");
  };
...
void loop() {
  ...
  if(active&&lastBrowserHeartbeat!=0&&millis()-lastBrowserHeartbeat>WEB_LEASE_MS)state=STOPPING;
  updateOperation(); delay(1);
}
